    ...
    var doDraw = function () {
    	...
    	for(var i = 0; i < drop.length; i++) { 		//loop over drop
			...
			if (soundLevel >= min_freq)
				drop[i]++;      //increment drop Y coordinate to create animation effect
			else
				drop[i]--;

            if (drop[i]*font_size > c.height)
                drop[i] = 0;	  	//reset if we reached the bottom or we don't have sound for that freq
            else if (drop[i]*font_size < 0)
            	drop[i] = c.height-1;
    	}
